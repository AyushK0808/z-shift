...
from two-dimensional observations without calibration or pose. A deployable
2D-to-3D system, however, needs more than a reconstruction network: ingestion,
frame selection, temporal synchronization, geometric refinement, and
application-specific delivery must all be present, and must work together. We
present z-shift, a four-stage pipeline whose stages are independently
replaceable and communicate only through serializable artifacts. Images,
videos, synchronized captures, and live frames are normalized by ingestion
into a unified spatial schema. Reconstruction wraps MASt3R with optional TSDF
fusion and a point-map fallback, refinement applies a fixed operator sequence,
and delivery routes GLB or point-cloud outputs under a compatibility contract.
Our evaluation combines a controlled benchmark suite with one eight-view
capture traced end to end. Unless capture order is restored, motion-ranked
frame retention degrades sliding-window pairing locality by 6.6x, at no cost
to fix. Mesh fragmentation governs refinement cost more than triangle count
does, behaving as a threshold: nearly flat below roughly 100 components, then
close to linear, reaching 23x at 1,000. Repeat runs of one configuration on
one GPU were byte-identical. Component- and integration-level correctness is
established across the pipeline. Real-time delivery, efficient large-scale
mesh processing, and quantitative reconstruction accuracy against ground truth
remain future work.
\end{abstract}
...
Input in the field is rarely uniform. Photographs, image directories, videos,
synchronized multi-camera captures, and continuous frame streams reach the same
entry point, and video contributes both redundancy and cost. To manage this,
z-shift normalizes the sources into a common spatial schema and bounds the
frames admitted to reconstruction (Section~\ref{sec:architecture}). Nor is raw
reconstruction output ready for production. Disconnected components, holes,
noise, and inconsistent normals may all appear in the surface. Established
mesh-processing methods address all four \cite{b16}--\cite{b19}; z-shift applies
...
Individually, the reconstruction components surveyed above are well studied, and
they work. Deployment additionally requires the surrounding stages enumerated in
Section~\ref{sec:architecture}, and assembling them raises a distinct problem: a
stage may satisfy its own specification and still violate an assumption the next
stage makes, or exhibit a cost profile that no stage shows alone. What we
address is the correctness and cost of the interfaces between those stages. The
accuracy of whichever model occupies the reconstruction slot is a separate
question.
...
reconstruction, mesh refinement, and outcome routing. Communication between them
passes only through serializable artifacts, namely a unified schema, a
reconstruction job, a mesh file, and a set of manifests. These four artifacts
are the system's interfaces. Each defines what one stage may assume about the
previous one, and each is where the behavior reported in
Section~\ref{sec:bench-validation} originates. Fig.~\ref{fig:overview}
summarizes the flow.

\subsection{Media Ingestion}

Through an HTTP/WebSocket gateway with pluggable authentication and rate limiting,
ingestion accepts single images, image folders, videos, video folders, and live
WebSocket frames. MIME type and file extension determine which processing track
a payload enters. Mixed or unrecognized payloads are rejected outright, never
partially processed. Normalization then follows on the batch track: images are
resized with aspect ratio and EXIF metadata preserved, while videos are probed
with FFmpeg and sampled by a motion-adaptive sampler whose interval shortens
during motion and lengthens across static spans. Where the input is a video
folder, a multi-source synchronizer estimates a per-source clock offset by
matching motion signatures against the source with the most frames, then aligns
frames to that anchor by nearest timestamp. The live track buffers frames
instead, in a bounded ring buffer with explicit backpressure. Oversized payloads
are refused, and a full buffer evicts its oldest frame. Compute priority goes
first to live work.
...
synchronization state. Two things are gained at this boundary, the first in the
pipeline. Ingestion is decoupled from reconstruction, so that neither stage
encodes assumptions about the other's internals. A normalized input also becomes
replayable, which lets reconstruction experiments run against a fixed frame set
instead of the original capture.

\subsection{Multi-View Reconstruction}
\label{sec:reconstruction}

An explicit reconstruction job is what the reconstruction stage makes of the
schema. Modes follow from source type: image folder to multi-view, single video
to video-sequence, video folder to synchronized-views. Because alignment
requires at least two views of one subject, single images and live streams are
rejected. Redundancy in video motivates a configurable frame budget for the job.
Beyond that budget the highest-motion frames are retained in preference to a
uniform subsample, and for synchronized captures the budget applies to whole
groups, leaving cross-camera groups intact. Pairing then proceeds by complete
scene graph or by sliding window, the latter preferred for videos and larger
image sets; on synchronized-views jobs, cross-camera pairs form within each
group so that simultaneous observations are matched directly. Frames arrive at
pairing in motion-rank order, not capture order.
Section~\ref{sec:bench-validation} measures this cross-stage interaction.

Before alignment, the pipeline resolves an execution device across CUDA, CPU, and
Metal backends, seeds the Python, NumPy, and PyTorch generators where a seed is
supplied, and converts an EXIF 35~mm-equivalent focal length into the per-image
intrinsic matrix. Sparse global alignment is then performed by MASt3R, with
intermediate results cached on disk so that repeated runs over the same frames
reuse prior work. TSDF fusion may optionally precede geometry export. Should
fusion be disabled, or should it fail under memory or runtime pressure, the
pipeline falls back to a dense point-map export and records the fallback. GLB is
the exporter's default for the colored mesh, with OBJ and PLY also supported,
and a confidence threshold discards low-confidence points. A confidence-masked
dense point cloud is written alongside it. Recorded in the run manifest are the
model, device, input resolution, pairing strategy, thresholds, seed, and
reproducibility metadata.

\subsection{Mesh Refinement}

Where an interactive tool would leave operator and threshold choices to a human on
a per-model basis, the refinement stage applies one fixed sequence. Validation
comes first: container formats such as GLB scenes are unwrapped, and meshes that
are empty or contain non-finite coordinates are rejected. Mode then governs
component handling. Under object mode, only the largest connected component
survives. Room mode splits the mesh into bodies, keeps every component above a
minimum cell count, and merges the survivors, which preserves scenes
legitimately made of several pieces. Boundary holes are closed up to a maximum
size derived from the bounding-box diagonal. Where the geometry is itself thin
and open along one axis, closure is skipped, so that an intentionally planar
capture is not sealed.

Taubin smoothing handles surface noise, suppressing it without the shrinkage that
repeated Laplacian passes introduce; under room mode, feature edges above a
configurable angle are additionally preserved. Finalization merges coincident
points, triangulates, optionally decimates to a target reduction, and recomputes
outward-facing normals. Since smoothing and decimation rewrite the vertex set,
vertex colors are carried onto the cleaned mesh by nearest-point transfer, and
appearance thereby survives the geometric operations. The interface between
these two stages must hold that invariant, and
Section~\ref{sec:bench-validation} checks it. Open boundary edges are counted in
a final pass, which flags the mesh watertight or not and treats a non-watertight
result as a warning that does not terminate the workflow. A refinement manifest
records input and output counts, the watertightness result, and any warnings.
